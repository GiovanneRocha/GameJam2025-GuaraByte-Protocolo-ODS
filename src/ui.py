...
import assets  # import do módulo de assets
...
def draw_instructions(screen, current_input):
    ...
    if guara_sprite:
        try:
            # aumentei o tamanho base de 40 -> 64 para ficar mais visível; ajusta se quiser
            guara_sprite_s = pygame.transform.scale(guara_sprite, (int(100 * S.SCALE), int(100 * S.SCALE)))
        except Exception:
            guara_sprite_s = guara_sprite
        guara_rect = guara_sprite_s.get_rect()
        # posiciona logo acima do texto de input
        guara_rect.midbottom = (S.WIDTH // 2, S.HEIGHT - int(84*S.SCALE) - 8)
        screen.blit(guara_sprite_s, guara_rect)

    # Nota: não usar 'surface' aqui, usamos 'screen' passado como parâmetro
