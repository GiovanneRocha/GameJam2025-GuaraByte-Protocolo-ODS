...
class Game:
    def __init__(self, record_demo_seconds: int | None = None):
        ...
        w = int(h * 9/16)
        os.makedirs(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'demo'), exist_ok=True)
        demo_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'demo')
        S.init_runtime(w, h, demo_dir)
        ...
        # gráfica / áudio por fase
        self.wallpaper = None   # Surface da fase atual (ou None)
        self._wallpaper_scaled = None  # cached scaled surface
    ...
    def begin_level(self, i: int):
        ...
        if getattr(cfg, 'wallpaper', None):
            try:
                self.wallpaper = assets.get_image(cfg.wallpaper)
            except Exception:
                self.wallpaper = None
    ...
    def run(self):
        ...
        while running:
            ...
            # Demo recording
            if self.record_seconds is not None and Image is not None:
                raw = pygame.image.tostring(self.screen, 'RGB')
                img = Image.frombytes('RGB', (S.WIDTH, S.HEIGHT), raw)
                self.record_frames.append(img)
                elapsed += dt
                if elapsed >= self.record_seconds:
                    out = os.path.join(S.DEMO_DIR or os.getcwd(), 'demo.gif')
                    frames = self.record_frames[::2] if len(self.record_frames) > 1 else self.record_frames
                    if frames:
